...
seedDat_spp <- seedDat[,.(T50_mean=mean(T50,na.rm=T),log_T50_mean=mean(log(T50),na.rm=T),GP = sum(GP,na.rm=T), sqrt_GP = mean(sqrt(GP),na.rm=T),Seedmass = mean(SM,na.rm=T), Moisture = mean(MC,na.rm=T)), by="Species"]
