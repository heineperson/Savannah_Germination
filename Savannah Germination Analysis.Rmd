...
Note: I was confused why the inverse matrix doesn't have each pairwise combination of phylogenetic distance represented. It seems that Hadfield (2010) argues this is too computationally intensive: https://onlinelibrary.wiley.com/doi/full/10.1111/j.1420-9101.2009.01915.x However, I think maybe I could coerce this through nodes="ALL" argument in inverseA. It seems like these things should not be happening by default but I think they are.
...
# Function for Evaluating MCMCmodel objects
evaluateMCMC <- function(modelObject){
 # objPlot <- plot(modelObject)
  objSumm <- summary(modelObject)
  ...
  objDIC <- summary(modelObject)$DIC

  return(list(objPlot,Summ=objSumm,lambdaVec=objlambdaVec,lambdaMean=objlambdaMean,lambdaMode=objlambdaMode,lambdaInt=objlambdaInt))
}
...
plot(model_T50)
evaluateT50 <- evaluateMCMC(model_T50)
...
model_T50_spp<-MCMCglmm(log(T50_mean)~1,random=~Species,
                    ...
                    data=seedDat_spp,nitt=3000000,burnin=100000,thin=500)
...
model_T50_fullmodel<-MCMCglmm(log(T50)~VegType + DM + MC + SM+Dormancy2Cat,random=~Species,family="gaussian",ginverse=list(Species=Ainv),prior=prior,
                    data=seedDat,nitt=500000,burnin=10000,thin=50)
...
lambda_dormOnly <- model_T50_dormancyCatOnly$VCV[,'Species']/
  (model_T50_dormancyCatOnly$VCV[,'Species']+model_T50_dormancyCatOnly$VCV[,'units'])
mean(lambda_dormOnly)
posterior.mode(lambda_dormOnly)
HPDinterval(lambda_dormOnly)
DICfull <- summary(lambda_dormOnly)$DIC
